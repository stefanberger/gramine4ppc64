...
// tls_base == NULL: switch to the shim_tcb if there is a glibc TCB set
// tls_base != NULL: assume tls_base is from glibc, so we set it and
//                   connect the shim_tcb to it via its LibOS_TCB pointer
static inline void* shim_tcb_set_tls_base(unsigned long tls_base, shim_tcb_t* tcb) {
    ...
    if (tls_base == 0) {
        ...
        PAL_TCB* r13_ptcb = (PAL_TCB*)tls_base;
...
static inline void shim_arch_update_tls_base(unsigned long tls_base, shim_tcb_t *shim_tcb) {
    shim_tcb_set_tls_base(tls_base, shim_tcb);
}
