...
shim_fp shim_table[LIBOS_SYSCALL_BOUND] = {
    ...
    (shim_fp)0,
    (shim_fp)0,
    (shim_fp)shim_do_semtimedop_time64,		/* 420 */
    (shim_fp)0,
...
noreturn void shim_table_dispatch(PAL_CONTEXT *context) {
...
#if 0
    debug("Calling Syscall %lu.  r3=0x%lx r4=0x%lx r5=0x%lx r6=0x%lx r7=0x%lx r8=0x%lx\n",
          sysnr, ALL_SYSCALL_ARGS(context));
    debug("%s: SYSCALLNR: %ld, FUNCTION to return to: 0x%lx\n", __func__, syscallnr, context->gpregs.link);
#endif
    ...
    switch (syscallnr) {
    case 2:
        log_debug("fork syscall (%ld) not supported. HALTING\n", syscallnr);
        while(1);
        break;
    }
