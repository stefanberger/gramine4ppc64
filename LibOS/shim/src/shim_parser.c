...
static void parse_open_flags(va_list VA_AP(ap));
static void parse_open_mode(va_list VA_AP(ap));
static void parse_access_mode(va_list VA_AP(ap));
static void parse_clone_flags(va_list VA_AP(ap));
static void parse_mmap_prot(va_list VA_AP(ap));
static void parse_mmap_flags(va_list VA_AP(ap));
static void parse_exec_args(va_list VA_AP(ap));
static void parse_exec_envp(va_list VA_AP(ap));
static void parse_pipe_fds(va_list VA_AP(ap));
static void parse_signum(va_list VA_AP(ap));
static void parse_sigmask(va_list VA_AP(ap));
static void parse_sigprocmask_how(va_list VA_AP(ap));
static void parse_timespec(va_list VA_AP(ap));
static void parse_sockaddr(va_list VA_AP(ap));
static void parse_domain(va_list VA_AP(ap));
static void parse_socktype(va_list VA_AP(ap));
static void parse_futexop(va_list VA_AP(ap));
static void parse_ioctlop(va_list VA_AP(ap));
static void parse_fcntlop(va_list VA_AP(ap));
static void parse_seek(va_list VA_AP(ap));
static void parse_at_fdcwd(va_list VA_AP(ap));
static void parse_wait_option(va_list VA_AP(ap));

struct parser_table {
    int slow;
    int stop;
    void (*parser[6])(va_list VA_AP(ap));
...
static inline void parse_string_arg(va_list VA_AP(ap)) {
    va_list ap_test_arg;
    va_copy(ap_test_arg, VA_AP(ap));
    const char* test_arg = va_arg(ap_test_arg, const char*);
    if (!test_user_string(test_arg)) {
        const char *s = va_arg(VA_AP(ap), const char *);
        PRINTF("\"%s\"", s);
    } else {
        const char *s = va_arg(VA_AP(ap), const char *);
...
static inline void parse_pointer_arg(va_list VA_AP(ap)) {
    void *p = va_arg(VA_AP(ap), void *);
    PRINTF("%p", p);
}

static inline void parse_integer_arg(va_list VA_AP(ap)) {
    int i = va_arg(VA_AP(ap), int);
    PRINTF("%d", i);
}

static inline void parse_syscall_args(va_list VA_AP(ap)) {
    const char* arg_type = va_arg(VA_AP(ap), const char*);
...
static inline void skip_syscall_args(va_list VA_AP(ap)) {
    const char* arg_type = va_arg(VA_AP(ap), const char*);

    if (!strcmp_static(arg_type, "const char *") || !strcmp_static(arg_type, "const char*"))
        va_arg(VA_AP(ap), const char*);
    else if (is_pointer(arg_type))
        va_arg(VA_AP(ap), void*);
    else
        va_arg(VA_AP(ap), int);
}
...
void parse_syscall_before(int sysno, const char* name, int nr, ...) {
    ...
    for (int i = 0; i < nr; i++) {
        ...
        if (parser->parser[i]) {
            ...
            (*parser->parser[i])(VA_AP_PARAM(ap));
        } else {
            parse_syscall_args(VA_AP_PARAM(ap));
        }
    }
    ...
    if (!parser->slow || parser->stop)
        for (int i = 0; i < nr; i++) {
            if (parser->stop && i < parser->stop) {
                skip_syscall_args(VA_AP_PARAM(ap));
                continue;
            }
...
static void parse_open_mode(va_list VA_AP(ap)) {
    mode_t mode = va_arg(VA_AP(ap), mode_t);
    PRINTF("%04o", mode);
}

static void parse_access_mode(va_list VA_AP(ap)) {
    int mode = va_arg(VA_AP(ap), int);
...
static void parse_exec_args(va_list VA_AP(ap)) {
    const char** args = va_arg(VA_AP(ap), const char**);
...
static void parse_exec_envp(va_list VA_AP(ap)) {
    const char** envp = va_arg(VA_AP(ap), const char**);
...
static void parse_signum(va_list VA_AP(ap)) {
    int signum = va_arg(VA_AP(ap), int);
...
static void parse_sigmask(va_list VA_AP(ap)) {
    __sigset_t* sigset = va_arg(VA_AP(ap), __sigset_t*);
...
static void parse_timespec(va_list VA_AP(ap)) {
    const struct timespec* tv = va_arg(VA_AP(ap), const struct timespec*);
...
static void parse_sockaddr(va_list VA_AP(ap)) {
    const struct sockaddr* addr = va_arg(VA_AP(ap), const struct sockaddr*);
...
static void parse_domain(va_list VA_AP(ap)) {
    int domain = va_arg(VA_AP(ap), int);
...
static void parse_socktype(va_list VA_AP(ap)) {
    int socktype = va_arg(VA_AP(ap), int);
...
static void parse_ioctlop(va_list VA_AP(ap)) {
    ...
    long unsigned int op = va_arg(VA_AP(ap), long unsigned int);
#endif
...
static void parse_wait_option(va_list VA_AP(ap)) {
    int option = va_arg(VA_AP(ap), int);
