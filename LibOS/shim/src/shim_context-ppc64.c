...
noreturn void restore_child_context_after_clone(struct shim_context* context, bool is_process) {
    ...
    // FIXME: This is necessary but should NOT be!
    ...
    shim_r13_set_tcb(context->regs->gpregs.gpr[13], tcb);
    if(context->regs->gpregs.gpr[13] != tls) {
        // never gets here!
        while (1);
    }
...
#if 0
    debug("restore context: SP = 0x%08lx, IP = 0x%08lx, R2 = 0x%lx, R3 = 0x%lx, R4 = 0x%lx, R5 = 0x%lx, R12 = 0x%lx, R13 = 0x%lx\n",
    ...
    debug("restore context: vrsave: 0x%lx fpscr: 0x%lx vscr: 0x%08lx%08lx\n",
...
noreturn void rt_sigreturn_caller(PAL_CONTEXT* context) {
    /* synthesise a call to rt_sigreturn; do NOT call directly! */
    context->gpregs.gpr[GLIBC_SYSCALL_NR_REGISTER] = __NR_rt_sigreturn;
...
void restart_syscall(PAL_CONTEXT* context, uint64_t syscall_nr) {
    ...
    context->gpregs.gpr[GLIBC_SYSCALL_NR_REGISTER] = syscall_nr;
