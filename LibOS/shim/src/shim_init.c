...
void * allocate_stack (size_t size, size_t protect_size, bool user)
{
    ...
    if (bkeep_mprotect(stack, size, PROT_READ|PROT_WRITE, !!(flags & VMA_INTERNAL)) < 0)
        return NULL;
#else
    // with the signaler handler wrappers we don't need this here anymore:
    ...
    if (bkeep_mprotect(stack, size, PROT_READ|PROT_WRITE|PROT_EXEC, !!(flags & VMA_INTERNAL)) < 0)
        return NULL;
#endif
