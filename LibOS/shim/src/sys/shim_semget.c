...
#if defined(__powerpc64__)
/* Also see ksys_ipc:
   https://elixir.bootlin.com/linux/v5.9/source/ipc/syscall.c#L20
 */
long shim_do_ipc(unsigned int call, int a1, unsigned long a2, unsigned long a3, void* ptr,long a5) {
    int version;
    struct ipc_kludge* k;
    ...
    case MSGRCV:
        ...
        if (version == 0) {
            k = ptr;
            if (!is_user_memory_readable(k, sizeof(*k)))
                return -EFAULT;
    ...
    default:
        log_debug("ipc call %d is not supported\n", call);
        return -ENOSYS;
    }
}
#endif /* __powerpc64__*/
