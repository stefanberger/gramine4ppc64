...
static int clone_implementation_wrapper(struct shim_clone_args* arg) {
    ...
    uint64_t tls_base = arg->tls_base;
    ...
    fixup_child_context(tcb->context.regs, stack, tls_base);
...
long shim_do_clone (unsigned long flags, unsigned long user_stack_addr, int* parent_tidptr,
    ...
    if (flags & CLONE_SETTLS) {
        // Note: On ppc64 'tls' is ready to be loaded into r13, thus r13 - 0x7000 - sizeof(tcbhead_t)
        //       would give us glib'c tcbhead_t pointer.
        tls_base = tls;
    }
...
#if defined(__powerpc64__)
    memcpy(&new_args.regs, self->shim_tcb->context.regs, sizeof(new_args.regs)); // FIXME: ubsan
#else
