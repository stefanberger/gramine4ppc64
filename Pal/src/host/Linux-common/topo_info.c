...
static int get_cache_topo_info(size_t cache_indices_cnt, size_t core_idx,
    ...
    for (size_t cache_idx = 0; cache_idx < cache_indices_cnt; cache_idx++) {
        ...
        READ_FILE_BUFFER(filename, cache_info->coherency_line_size,
...
skip1:
        ...
        snprintf(filename, sizeof(filename),
                 ...
                 cache_idx);
        READ_FILE_BUFFER(filename, cache_info->number_of_sets,
#if defined(__x86_64__)
                         /*failure_label=*/out_cache);
#elif defined(__powerpc64__)
        /* FIXME: not always available */
...
static int get_core_topo_info(struct pal_topo_info* topo_info) {
    ...
    step = 8;
    memset(core_topology_arr, 0, sizeof(struct pal_core_topo_info));
    memcpy(core_topology_arr[0].is_logical_core_online, "1\n", 2);
#endif
    for (size_t idx = 0; idx < online_logical_cores_cnt; idx+=step) {
        ...
        if (ret < 0)
            goto out_topology;
        for (size_t j = idx + 1; j < idx + step && j < online_logical_cores_cnt; j++)
            core_topology_arr[j] = core_topology_arr[idx];
    }
...
static int get_numa_topo_info(struct pal_topo_info* topo_info) {
    ...
    for (size_t idx = 0; idx < online_nodes_cnt; idx++, sidx++) {
        while (1) {
            snprintf(filename, sizeof(filename), "/sys/devices/system/node/node%zu/cpumap", sidx);
            READ_FILE_BUFFER(filename, numa_topology_arr[idx].cpumap, /*failure_label=*/next_sidx);
            break;
next_sidx:
            sidx++;
            if (sidx == 2048)
        ...
        snprintf(filename, sizeof(filename), "/sys/devices/system/node/node%zu/distance", sidx);
