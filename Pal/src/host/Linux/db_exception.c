 ...
 */
static void perform_signal_handling(int event, bool is_in_pal, PAL_NUM addr, ucontext_t* uc, siginfo_t *info) {
...
SIGHANDLER_FUNCTION(handle_sync_signal)(int signum, siginfo_t* info, struct ucontext* uc) {
    ...
    if (!ADDR_IN_PAL(rip)) {
        ...
        perform_signal_handling(event, /*is_in_pal=*/false, (PAL_NUM)info->si_addr, uc, info);
        return;
    }
...
SIGHANDLER_FUNCTION(handle_async_signal)(int signum, siginfo_t* info, struct ucontext* uc) {
    ...
    uintptr_t rip = ucontext_get_ip(uc);
    perform_signal_handling(event, ADDR_IN_PAL(rip), /*addr=*/0, uc, info);
}
