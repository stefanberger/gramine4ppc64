...
static long get_offset_by_vaddr(ElfW(Addr) ehdr_addr, ElfW(Addr) vaddr,
                                int in_memory)
{
    if (!in_memory)
        return 0;
...
static ElfW(Dyn)* find_dynamic_section(ElfW(Addr) ehdr_addr, ElfW(Addr) base_diff,
    ...
    for (const ElfW(Phdr)* ph = phdr; ph < &phdr[header->e_phnum]; ph++) {
        if (ph->p_type == PT_DYNAMIC) {
            offset = get_offset_by_vaddr(ehdr_addr, ph->p_vaddr, in_memory);
            dynamic_section = (ElfW(Dyn)*)(ph->p_vaddr + base_diff + offset);
            break;
        }
    }
...
int find_string_and_symbol_tables(ElfW(Addr) ehdr_addr, ElfW(Addr) base_diff,
    ...
    ElfW(Dyn)* dynamic_section = find_dynamic_section(ehdr_addr, base_diff, in_memory);
    if (!dynamic_section) {
    ...
    while (dynamic_section_entry->d_tag != DT_NULL) {
        switch (dynamic_section_entry->d_tag) {
            case DT_STRTAB:
                offset = get_offset_by_vaddr(ehdr_addr, dynamic_section_entry->d_un.d_ptr, in_memory);
                string_table = (const char*)(dynamic_section_entry->d_un.d_ptr + offset +base_diff);
                break;
            case DT_SYMTAB:
                offset = get_offset_by_vaddr(ehdr_addr, dynamic_section_entry->d_un.d_ptr, in_memory);
                symbol_table = (ElfW(Sym)*)(dynamic_section_entry->d_un.d_ptr + offset + base_diff);
                break;
            case DT_HASH: {
                /* symbol table size can only be found via ELF hash table's nchain (which is the
                 * second word in the ELF hash table struct) */
                offset = get_offset_by_vaddr(ehdr_addr, dynamic_section_entry->d_un.d_ptr, in_memory);
                ElfW(Word)* ht = (ElfW(Word)*)(dynamic_section_entry->d_un.d_ptr + offset +base_diff);
    ...
    long start = ALIGN_DOWN_POW2((long)symbol_table, 65536);
    long end = ALIGN_UP_POW2((long)symbol_table + symbol_table_cnt * sizeof(ElfW(Sym)), 65536);
    int ret = _DkVirtualMemoryProtect((void*)start, end - start, PAL_PROT_READ | PAL_PROT_WRITE | PAL_PROT_EXEC);
    if (ret)
        log_error("BAD: ret=%d\n", ret);
    /* adjust the symbol table */
    uint32_t i;
    for (i = 0; i < symbol_table_cnt; i++) {
        offset = get_offset_by_vaddr(ehdr_addr, symbol_table[i].st_value, in_memory);
        symbol_table[i].st_value += offset;
    }
    ret = _DkVirtualMemoryProtect((void*)start, end - start, PAL_PROT_READ | PAL_PROT_EXEC);
    if (ret)
        log_error("BAD: ret=%d\n", ret);
#endif
...
//    log_error("symbol_idx: %ld  name: %s\n", symbol_idx, symbol_name);
...
//                log_error("--> 0x%lx\n", loaded_map->symbol_table[i].st_value);
            return 0;
        }
    }
...
static int perform_relocations(struct link_map* map) {
    ...
    while (dynamic_section_entry->d_tag != DT_NULL) {
        switch (dynamic_section_entry->d_tag) {
            case DT_RELA:
                offset = get_offset_by_vaddr(base_diff, dynamic_section_entry->d_un.d_ptr, map->in_memory);
                relas_addr = (ElfW(Rela)*)(dynamic_section_entry->d_un.d_ptr + offset + base_diff);
                break;
            case DT_RELASZ:
            ...
            case DT_JMPREL:
                offset = get_offset_by_vaddr(base_diff, dynamic_section_entry->d_un.d_ptr, map->in_memory);
                plt_relas_addr = (ElfW(Rela)*)(dynamic_section_entry->d_un.d_ptr + offset + base_diff);
                break;
            case DT_PLTRELSZ:
    ...
    for (ElfW(Rela)* rela = relas_addr; rela < relas_addr_end; rela++) {
        ...
        if (ELFW(R_TYPE)(rela->r_info) == R_PPC64_RELATIVE) {
            // FIXME: offset seems to 'work like this' but is it always correct?
            ElfW(Addr)* addr_to_relocate = (ElfW(Addr)*)(rela->r_offset + base_diff + offset);
            *addr_to_relocate = *addr_to_relocate + base_diff + offset;
...
#elif defined(__powerpc64__)
        if (ELFW(R_TYPE)(plt_rela->r_info) != R_PPC64_JMP_SLOT) {
            log_error("Unrecognized relocation type; PAL loader currently supports only "
...
#if 0
        log_error("addr: %p symbol: 0x%lx addend: 0x%lx in_memory: %d\n",
                  addr_to_relocate, symbol_addr, plt_rela->r_addend, map->in_memory);
#endif
    }
...
noreturn void start_execution(const char** arguments, const char** environs) {
    ...
    __asm__ volatile("movq %1, %%rsp\n"
                     ...
                     : "memory");
#elif defined __powerpc64__
    // FIXME: Should call DkProcessExit but if adding if here the PAL binary cannot be relocated
    // This call to exit_group / DkProcessExit could go into Pal/regression/crt_init/arch/ppc64/user_start.S
