...
static inline PAL_TCB* pal_set_PAL_TCB(PAL_TCB *ptcb) {
    return pal_set_r13(ptcb);
}
...
/*
 * Vararg parameters: On ppc64 a va_list parameter must be processed in the funtion is was received
 *                    or must be passed to other functions using a pointer -> 'va_list *ap'
 *                    Therefore, VA_AP is *ap and functions use 'va_list VA_AP' in their signature.
 */
#define VA_AP(AP)       *(AP)
#define VA_AP_PARAM(AP) &(AP)
