...
static inline void ucontext_to_pal_context(PAL_CONTEXT* context, ucontext_t* uc) {
    ...
    /* vscr: this one behaves strangely also on Linux */
    memcpy(&context->xtregs.vscr, &v_regs->vscr, sizeof(v_regs->vscr));
}
...
static inline void pal_context_to_ucontext(ucontext_t* uc, PAL_CONTEXT* context) {
    ...
    /* vscr: this one behaves strangely also on Linux */
    memcpy(&v_regs->vscr, &context->xtregs.vscr, sizeof(v_regs->vscr));
}
