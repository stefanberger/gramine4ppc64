...
#if !defined(__i386__)
static inline unsigned long long getuint(va_list VA_AP(ap), int lflag)
#else
static inline unsigned long getuint(va_list VA_AP(ap), int lflag)
#endif
{
#if !defined(__i386__)
    if (lflag >= 2)
        return va_arg(VA_AP(ap), unsigned long long);
#endif
    if (lflag)
        return va_arg(VA_AP(ap), unsigned long);
    return va_arg(VA_AP(ap), unsigned int);
}

// Same as getuint but signed - can't use getuint
// because of sign extension
#if !defined(__i386__)
static inline long long getint(va_list VA_AP(ap), int lflag)
#else
static inline long getint(va_list VA_AP(ap), int lflag)
#endif
{
#if !defined(__i386__)
    if (lflag >= 2)
        return va_arg(VA_AP(ap), long long);
#endif
    if (lflag)
        return va_arg(VA_AP(ap), long);
    return va_arg(VA_AP(ap), int);
}
...
void vfprintfmt(int (*_fputch)(void*, int, void*), void* f, void* putdat, const char* fmt,
    ...
    reswitch:
        switch (ch = *(unsigned char*)(fmt++)) {
            ...
            case 'd':
            case 'i':
                num = getint(VA_AP_PARAM(ap), lflag);
#if !defined(__i386__)
                if ((long long)num < 0) {
                    if ((*_fputch)(f, '-', putdat) == -1)
                        return;
            ...
            // unsigned decimal
            case 'u':
                num  = getuint(VA_AP_PARAM(ap), lflag);
                base = 10;
                goto number;
